...
<aside id="sidebar" class="sidebar overflow-auto">
    @php
        ...
        // Decide which route prefix to use for sale/logs links
        $salePrefix = $role === 'admin' ? 'admin' : 'user';

        $saleIndexRoute      = route($salePrefix . '.sale-orders.index');
        $saleTransRoute      = route($salePrefix . '.sale-orders.transactions');
        $logsRoute           = route($salePrefix . '.logs.index');
        ...
        $logsRouteIs         = request()->routeIs('admin.logs.index', 'user.logs.index');
    @endphp
    <div class="logo-area">
...
<ul class="nav flex-column mb-10">
        ...
        {{-- MAINTENANCE:
             - Admin: Reports + Logs
             - Cashier: only Logs
             - Inventory: nothing here (they have inventory menus in admin sidebar) --}}
        @if($role === 'admin' || $role === 'cashier')
